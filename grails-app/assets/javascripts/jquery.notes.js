...
(function($) {
	...
	var _createHtml = function(pointer) {
	    ...
	    $('#jquery-notes_'+pointer+' img').dblclick(function(event) {
	    _startAdd(pointer);
	    _addNote(pointer,event);
	    _setNote(pointer,event);
	    });
	...
	var _printNote = function(pointer, note) {
    	...
    	$('#jquery-notes_'+pointer+' .notes #n_'+pointer+'-'+note.ID).dblclick(function(){
			_openSettings(pointer, note.ID)
		});
	...
	var _setNote = function(pointer, event) {
		...
		html += '<textarea name="note" placeholder="Tên vùng"></textarea><br />';
