<table id="dm-Item-table-1"
       class="table table-striped table-bordered table-hover ">
</table>

<div id="loiDuLieu_itemTable" class="alert alert-info" style="max-width: 600px; margin: auto; display: none">
...
<script type="text/javascript">
    ...
    window.tableDmItem_1 = $("#dm-Item-table-1").DataTable({
        processing: true,
        ...
        columns: [
            ...
            },
            {
                ...
                render: function (data, type, row, meta) {
                    return "<a href='#' class='editRowCH_TL icon-edit' title='Edit'></a> <a href='#' class='deleteRowCH_TL icon-trash' title='Delete'></a> "
                }
            }
        ],
    ...
    $('#dm-Item-table-1').on('click', 'a.deleteRowCH_TL', function (e) {
        e.preventDefault();
        ...
        common.confirm(null, null, 'M', 'Xác nhận', 'Bạn chắc chắn muốn xóa dòng này?', function (result) {
            if (result) {
                dataDsCH_TL_Copy.forEach(function (item) {
                    if (item.cauHoiId == tableDmItem_1.row(deleteRow).data().cauHoiId && item.newRow == tableDmItem_1.row(deleteRow).data().newRow) {
                        window.dataDsCH_TLXoa.push(item);
                    }
                });
                dataDsCH_TL = window.dataDsCH_TL.filter(function (item) {
                    return (item.cauHoiId != tableDmItem_1.row(deleteRow).data().cauHoiId || item.newRow != tableDmItem_1.row(deleteRow).data().newRow);
                });

                tableDmItem_1.clear().rows.add(dataDsCH_TL).draw();
            }
        })
    });
    $('#dm-Item-table-1').on('click', 'a.editRowCH_TL, a.viewRowCH_TL', function (e) {
        e.preventDefault();

        $('#loiDuLieu_itemTable').html('').css('display', 'none');

        var selectRow = $(this).closest('tr');
        var objCH_TL = window.dataDsCH_TL.find(function (item) {
            return (item.cauHoiId == tableDmItem_1.row(selectRow).data().cauHoiId && item.newRow == tableDmItem_1.row(selectRow).data().newRow);
        })
        ...
        if ($(this).hasClass('viewRowCH_TL')) {
            $('#modal_Update button[name=form-button-save]').css('display', 'none');
        } else if ($(this).hasClass('editRowCH_TL')) {
    ...
    function hienThiLoi_ItemTable(dsLoi) {
        ...
        $('#loiDuLieu_itemTable').html(strHTML).css('display', 'block');
    }
</script>
<style>
#dm-Item-table-1 td, th {
    border-left: 0px !important;
    border-bottom: 0px !important;
}

#dm-Item-table-1_wrapper .dt-buttons {
    margin-left: 10px;
}

#dm-Item-table-1_wrapper .cuonNgang {
    overflow-x: auto;
}

/*#dm-Item-table-1_wrapper {*/
/*    width: 75% !important;*/
/*    margin:auto;*/
/*}*/
#dm-Item-table-1_wrapper .phiaTren {
    clear: both;
